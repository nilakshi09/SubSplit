      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        ...
        >
          ...
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-[#2D3748] font-bold text-xl">Settle Up</h2>
            <button
              onClick={onClose}
          ...
          {owedMembers.length === 0 ? (
            <div className="text-center py-10">
              ...
              <p className="text-[#2D3748] font-medium text-lg">You're all settled up!</p>
              <p className="text-[#718096] text-sm mt-1">No outstanding balances</p>
            </div>
          ) : (
            <>
              {/* Payment Method */}
              ...
              <div className="space-y-2">
                {owedMembers.map((member, i) => (
                  <motion.div
                    ...
                    className="flex items-center gap-3 bg-white/[0.03] rounded-xl p-3 border border-[#E2E8F0]"
                  >
                    {/* Avatar */}
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-teal-500/20 to-teal-600/10 border border-teal-500/20 flex items-center justify-center text-[#16a34a] font-semibold text-sm flex-shrink-0">
                      {member.name.charAt(0).toUpperCase()}
                    </div>

                    {/* Info */}
                    <div className="flex-1 min-w-0">
                      <p className="text-[#2D3748] text-sm font-medium truncate">{member.name}</p>
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 bg-[#4ADE80] hover:bg-teal-400 disabled:opacity-50 text-[#2D3748] text-xs font-semibold px-3.5 py-2 rounded-lg transition-colors cursor-pointer whitespace-nowrap"
                    >
